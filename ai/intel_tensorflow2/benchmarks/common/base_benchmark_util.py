...
class BaseBenchmarkUtil(object):
    ...
    def _define_args(self):
        ...
        models 为FP32和int8模型共享的基准接口定义参数"""
        ...
        self._common_arg_parser = ArgumentParser(
            add_help=False, description="Parse args for base benchmark "
                                        "interface 分析基本基准接口的参数")

        self._common_arg_parser.add_argument(
            "-f", "--framework",
            help="Specify the name of the deep learning framework to use.指定要使用的深度学习框架的名称。",
            dest="framework", default=None, required=required_arg)

        self._common_arg_parser.add_argument(
            "-r", "--model-source-dir",
            help="Specify the models source directory from your local machine 从本地计算机指定模型源目录",
