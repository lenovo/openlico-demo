...
def get_gpus(args):
    ...
    if args.gpu_number > 0:
       gpus = str(list(range(args.gpu_number)))[1:-1]
       return gpus
    else:
       if 'CUDA_VISIBLE_DEVICES' not in os.environ:
          return None
       gpus_str = os.environ['CUDA_VISIBLE_DEVICES']
       if not gpus_str or gpus_str == 'NoDevFiles':
          return None
       gpu_num = len(gpus_str.split(','))
       gpus = str(list(range(gpu_num)))[1:-1]
       return gpus
...
def read_data(label, image):
    ...
    data_dir = os.path.dirname(os.path.realpath(__file__)) + '/../../datasets/mnist/'
    #with gzip.open(download_file(base_url+label, os.path.join('data',label))) as flbl:
    with gzip.open(os.path.join(data_dir, 'train-labels-idx1-ubyte.gz')) as flbl:
        magic, num = struct.unpack(">II", flbl.read(8))
        label = np.fromstring(flbl.read(), dtype=np.int8)
    #with gzip.open(download_file(base_url+image, os.path.join('data',image)), 'rb') as fimg:
    with gzip.open(os.path.join(data_dir, 'train-images-idx3-ubyte.gz')) as fimg:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_number', type=int, default=0,
                        help='the number of setting gpu')
