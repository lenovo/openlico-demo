...
def main():
    ...
    dataset = tf.data.Dataset.from_tensor_slices(
        ((mnist_images[..., tf.newaxis] / 255.0).astype(np.float32), tf.cast(mnist_labels, tf.int32))
        #(tf.cast(mnist_images[..., tf.newaxis] / 255.0, tf.float32)), tf.cast(mnist_labels, tf.int32))
    )
    ...
    mnist_model.fit(dataset, steps_per_epoch=3000 // hvd.size(), callbacks=callbacks, epochs=10, verbose=verbose)
