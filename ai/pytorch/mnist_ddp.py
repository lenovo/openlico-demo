...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PyTorch MNIST Distributed Example')
    ...
    parser.add_argument("--dist-backend", default="nccl", type=str, help="communications backend")
    parser.add_argument("--gloo-socket-ifname", default="ib0", type=str, help="gloo socket ifname")
    parser.add_argument("--nccl-socket-ifname", default="ib0", type=str, help="nccl socket ifname")
    ...
    dataset_train = datasets.MNIST(os.path.dirname(os.path.realpath(__file__)) + '/data', train=True, download=True,
                       transform=transform)
    dataset_val = datasets.MNIST(os.path.dirname(os.path.realpath(__file__)) + '/data', train=False,
                       transform=transform)
    if args.distributed:
    ...
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu] if not args.no_cuda else None)
